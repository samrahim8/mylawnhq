...
export function LawnPlan() {
  ...
  return (
    <div className="space-y-3">
      {/* Header card */}
      <div className="bg-gradient-to-br from-lawn to-lawn/80 rounded-2xl p-4 sm:p-5 text-white">
        <div className="flex items-start justify-between mb-3">
          <div>
            <h2 className="font-display text-lg sm:text-xl font-bold">Your 90-Day Plan</h2>
            <p className="text-white/70 text-xs sm:text-sm mt-0.5">
              {grassLabels[params.grassType] || params.grassType} · {params.zip}
            </p>
          </div>
          <div className="text-right">
            <div className="text-xl sm:text-2xl font-bold">{progressPercent}%</div>
            <div className="text-xs text-white/60">{completedCount}/{totalTasks}</div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          <span className="inline-flex items-center bg-white/15 text-white text-xs font-medium px-2.5 py-1 rounded-full">
