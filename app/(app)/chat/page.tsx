...
function ChatContent() {
  ...
  useEffect(() => {
    if (currentSession && currentSession.messages.length > 0 && !initialQuestion) {
      setMessages(currentSession.messages);
      activeSessionId.current = currentSession.id;
    }
  ...
  // Persist messages to ChatContext session
  const persistMessages = (updatedMessages: ChatMessage[]) => {
    if (!activeSessionId.current) {
      const newId = createSession();
      activeSessionId.current = newId;
      setCurrentSessionId(newId);
    }
    updateSession(activeSessionId.current, updatedMessages);
  };
      ...
      <div className="bg-white border-t border-deep-brown/10 pb-[env(safe-area-inset-bottom)]">
        <div className="max-w-2xl mx-auto px-4 py-3">
          ...
          <div className="flex gap-2 items-end">
            ...
            <input ref={cameraInputRef} type="file" accept="image/jpeg,image/png" capture="environment" onChange={handleImageUpload} className="hidden" />
